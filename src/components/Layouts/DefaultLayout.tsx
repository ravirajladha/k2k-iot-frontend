...
import AlertComponent from "../../components/Alerts"; // Import the AlertComponent
...
const DefaultLayout = ({ children }: PropsWithChildren) => {
    ...
    const alerts = useSelector((state: IRootState) => state.alerts.alerts);
            ...
            <div className="relative">
                ...
                <div className={`${themeConfig.navbar} main-container text-black dark:text-white-dark min-h-screen`}>
                    ...
                    <div className="main-content flex flex-col min-h-screen">
                        ...
                        {/* END TOP NAVBAR */}
                        {/* ALERTS */}
                         {/* Later create the component for the alert too... */}
                        <div className="fixed top-4 right-4 z-50 space-y-4">
                            {alerts.map((alert) => (
                                <AlertComponent
                                    key={alert.id} // Unique key
                                    id={alert.id} // Pass id as a prop
                                    type={alert.type}
                                    message={alert.message}
                                    autoClose={alert.autoClose} // Pass autoClose duration
                                    onClose={() => dispatch(removeAlert(alert.id))} // Handle close
                                />
                            ))}
                        </div>
