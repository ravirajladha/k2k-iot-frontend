...
const ProjectCreation = () => {
  ...
  const [machineRole, setMachineRole] = useState<string>('');
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!selectedClient || !selectedFactory || !machineName || !machineRole) {
    ...
    try {
      ...
      const response = await fetch(`${baseURL}/machines`, {
        ...
        body: JSON.stringify({ clientId: selectedClient, factoryId: selectedFactory, machineRole, machineName, created_by: userId }),
      });
      ...
      navigate('/falcon-facade/machines');
    } catch (error) {
  ...
  const breadcrumbItems = [
    ...
    { label: 'Machines', link: '/falcon-facade/machines', isActive: false },
  ...
  return (
    <div>
      <Breadcrumbs
        ...
        addButton={{ label: 'Back', link: '/falcon-facade/machines', icon: <IconArrowBackward className="text-4xl" /> }}
      />
      <div className="panel">
        <div className="mb-5">
...
::contentReference[oaicite:0]{index=0}
            ...
            */}
              ...
              <div>
                <label htmlFor="name" className="w-1/4 pr-4">Machine Role</label>
                <input
                  id="machineName"
                  type="text"
                  value={machineRole}
                  className="form-input flex-1"
                  onChange={handleMachineNameChange}
                  placeholder="Enter Machine Role (Eg. Cutting, Bending)"
                  required
                />
              </div>
          ...
          <div className="flex justify-between space-x-4 mt-6">
            ...
            {/* Cancel Button */}
            <Link to="/falcon-facade/machines" className="flex-1">
