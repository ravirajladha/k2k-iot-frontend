...
const DispatchCreation = () => {
    ...
    const handleWorkOrderChange = (selectedOption) => {
        setSelectedWorkOrder(selectedOption?.value || null);
        setValue('workOrderNumber', selectedOption?.value || '');
        setScannedItems([]);
        setScannedQRCodes([]);
    };
    ...
    const handleQRCodeInput = async () => {
        ...
        try {
            ...
            if (existingItemIndex !== -1) {
                ...
                updatedItems[existingItemIndex].batchIds.push(qrId);
                updatedItems[existingItemIndex].dispatchQuantity += newItem.dispatchQuantity;
                setScannedItems(updatedItems);
            } else {
                setScannedItems([...scannedItems, newItem]);
    ...
    const onSubmit = async (formData) => {
        ...
        try {
            ...
            formData.uploads.forEach((fileObj) => {
                if (fileObj.file) {
                ...
                <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
                    <div className="grid grid-cols-1 sm:grid-cols-1 gap-4">
                        ...
                        <div>
                                ...
                                render={({ field }) => (
                                    <Select
                                        ...
                                        onChange={(selectedOption) => {
                                            handleWorkOrderChange(selectedOption);
                            ...
                            />
                            {errors.workOrderNumber && <p className="text-red-500 text-sm mt-1">{errors.workOrderNumber.message}</p>}
                        </div>
